...
export default function AddItemScreen() {
  ...
  const handleScanPhoto = async () => {
    ...
    if (status !== "granted") {
      Alert.alert(
        "Permission needed",
        "Please allow access to your photo library to scan clothing items."
      );
      return;
    }
    ...
    try {
      ...
      setSelectedColor(
        colorMatch ?? { name: scan.colorName, hex: scan.colorHex }
      );
      setSeasons(scan.seasons.filter((s): s is Season =>
      ...
      Alert.alert(
        "Scan failed",
        "Could not identify the item. Please fill in the details manually."
      );
  ...
  const handleCameraCapture = async () => {
    ...
    try {
      const scan = await identifyClothing(base64, asset.mimeType ?? "image/jpeg");

      setName(scan.name);
      setCategory(scan.category);
      const colorMatch = COLOR_SWATCHES.find(
        (c) => c.name.toLowerCase() === scan.colorName.toLowerCase()
      );
      ...
      Alert.alert("Scan failed", "Could not identify the item. Please fill in manually.");
      setScannedImage(null);
    } finally {
      setIsScanning(false);
    }
      ...
      >
        <View style={[styles.scanSection, { borderColor: colors.border, backgroundColor: colors.card }]}>
          ...
          <Text style={[styles.scanSubtitle, { color: colors.mutedForeground }]}>
            Take or upload a photo — we'll detect the color, type, and fabric automatically
          </Text>
          ...
          {scannedImage && (
            <View style={styles.scannedImageWrap}>
              ...
              {scanDone && !isScanning && (
                <Animated.View style={[styles.scanDoneBadge, { backgroundColor: colors.accent }]}>
          ...
          <Animated.View style={[styles.scanButtons, scanAnimStyle]}>
            <TouchableOpacity
              ...
              style={[
                styles.scanBtn,
                { backgroundColor: colors.primary, opacity: isScanning ? 0.6 : 1 },
              ]}
            >
        ...
        {material ? (
          ...
          >
            <View style={styles.materialRow}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={[styles.materialLabel, { color: colors.mutedForeground }]}>
                  FABRIC / MATERIAL
                </Text>
                <Text style={[styles.materialValue, { color: colors.foreground }]}>
                  {material}
                </Text>
              </View>
            </View>
            ...
            {CATEGORIES.map((c) => (
              ...
              >
                <Feather
                  ...
                  size={18}
                  color={
                    category === c.key ? colors.primaryForeground : colors.mutedForeground
                  }
                />
                <Text
                  style={[
        ...
        <View style={styles.section}>
          ...
          <View style={styles.colorGrid}>
            ...
            {selectedColor &&
              !COLOR_SWATCHES.find((c) => c.hex === selectedColor.hex) && (
                <View style={styles.colorItem}>
                  ...
                  >
                    <Feather
                      name="check"
                      size={14}
                      color={isLight(selectedColor.hex) ? "#1C1512" : "#FFFFFF"}
                    />
                  </View>
                  <Text
                    style={[styles.colorLabel, { color: colors.foreground }]}
                    numberOfLines={1}
                  >
                    {selectedColor.name}
                  </Text>

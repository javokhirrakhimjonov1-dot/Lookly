...
// ---------------------------------------------------------------------------
// Server-side storage helpers (for use in API routes that need direct upload,
// download, or delete — not for client-initiated presigned-URL flows)
// ---------------------------------------------------------------------------

/**
 * Upload a local file directly from the server to object storage.
 * Returns the canonical storage path (/<bucket>/<objectName>).
 */
 ...
 */
export async function serverDownloadFile(storagePath: string): Promise<Buffer> {
  const { bucketName, objectName } = parseObjectPath(storagePath);
  const bucket = objectStorageClient.bucket(bucketName);
...
/**
 * Delete an object from storage. Resolves silently if the object does not
 * exist (idempotent delete).
 */
export async function serverDeleteFile(storagePath: string): Promise<void> {
  const { bucketName, objectName } = parseObjectPath(storagePath);
  const bucket = objectStorageClient.bucket(bucketName);
  try {
    await bucket.file(objectName).delete();
  } catch (err: unknown) {
    // Treat 404 (object already gone) as success
    if (
      typeof err === "object" &&
      err !== null &&
      "code" in err &&

...
export interface StoredWardrobeItem { id: string; name: string; category: string; color?: string; colorHex?: string; seasons?: string[]; fabricWeight?: string; tags?: string[]; material?: string; timesWorn?: number; lastWornDaysAgo?: number; wearCount?: number; isWorkwear?: boolean; waterproof?: boolean; windproof?: boolean; warm?: boolean; fit?: Item["fit"]; visualSignature?: { garmentFamily?: string; itemType?: string; silhouette?: string; shape?: string; length?: string; sleeve?: string; neckline?: string; coverage?: string; opacity?: string; layerRole?: string; toeStyle?: string; heelType?: string; heelHeight?: string; bootShaft?: string }; }
...
export function mapWardrobeItem(item: StoredWardrobeItem): Item | null {
  ...
  return { id:item.id, name:item.name, category, color, hue:hueFromHex(item.colorHex,color), seasons:(Array.isArray(item.seasons)?item.seasons:[]).map((x)=>norm(x) as Season).filter((x): x is Season => seasons.has(x)), weight:weights.has(candidateWeight)?candidateWeight:"medium", tags, material:material(item.material,source), waterproof:inferred(item,"waterproof",source), windproof:inferred(item,"windproof",source), warm:inferred(item,"warm",source), fit:item.fit, garmentFamily:norm(signature.garmentFamily ?? signature.itemType) || undefined, silhouette:norm(signature.silhouette ?? signature.shape) || undefined, length:norm(signature.length) || undefined, sleeve:norm(signature.sleeve) || undefined, neckline:norm(signature.neckline) || undefined, coverage:norm(signature.coverage) || undefined, opacity:norm(signature.opacity) || undefined, layerRole:norm(signature.layerRole) || undefined, toeStyle:norm(signature.toeStyle) || undefined, heelType:norm(signature.heelType) || undefined, heelHeight:norm(signature.heelHeight) || undefined, bootShaft:norm(signature.bootShaft) || undefined, lastWornDaysAgo:Number.isFinite(item.lastWornDaysAgo)?item.lastWornDaysAgo:30, wearCount:Number.isFinite(item.wearCount)?item.wearCount:(item.timesWorn??0) };
}
